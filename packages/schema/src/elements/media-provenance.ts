 ...
 * Per ADR-008 §D2.
 *
 * T-438 extends the discriminator with `'asset-gen-pending'` — a
 * NON-TERMINAL state covering elements whose adapter dispatch is still
 * in flight. Editor-shell renders pending elements with a placeholder
 * visual (greyed-out box + spinner) and listens for the swap patch
 * that flips them to a terminal kind once the adapter resolves.
 * §13 structural extension; option 3 deferral (pixel verification
 * gated on T-442 downstream).
 */
export const MEDIA_PROVENANCE_KINDS = [
  ...
  'asset-gen-pending', // T-438 — adapter dispatch in flight; placeholder element
] as const;
...
export const mediaProvenanceSchema = z
  .object({
    ...
    /**
     * --- T-438 optimistic-placeholder slots ---
     *
     * Opaque correlation id linking a placeholder emission to the
     * eventual swap patch that replaces the placeholder element with
     * a terminal-state MediaElement. Set ONLY on elements whose
     * `kind === 'asset-gen-pending'`; downstream swap patches MUST
     * carry the same `placeholderId` so the editor-shell can match
     * placeholder → resolved asset across the swap.
     *
     * Generated by `generate_asset` (engine handler T-423) via
     * `crypto.randomUUID()`.
     */
    placeholderId: z.string().min(1).optional(),

    /**
     * ISO-8601 datetime upper-bound hint the editor uses for the
     * placeholder countdown / spinner. Derived from the chosen
     * adapter's optional `latencyHintMs` capability metadata; falls
     * back to a per-modality default. Optional even on pending
     * elements — when absent, the editor renders an indefinite
     * spinner.
     */
