// packages/pack-creator-style/scripts/build-pack.ts
// T-516 — Build script for `@stageflip/pack-creator-style`. Walks
...
// to set `STAGEFLIP_CREATOR_STYLE_OUT`.
//
...
if (isMainModule) {
  ...
  const keyPath = process.env.STAGEFLIP_CREATOR_STYLE_KEY
    ? resolve(process.env.STAGEFLIP_CREATOR_STYLE_KEY)
    : resolve(packageRoot, 'keys/creator-style-dev.private.pem');
  const sourceDir = process.env.STAGEFLIP_CREATOR_STYLE_SRC
    ? resolve(process.env.STAGEFLIP_CREATOR_STYLE_SRC)
    : resolve(packageRoot, 'packs');
  const outDir = process.env.STAGEFLIP_CREATOR_STYLE_OUT
    ? resolve(process.env.STAGEFLIP_CREATOR_STYLE_OUT)
    : resolve(packageRoot, `../../packs/stageflip/creator-style/${MANIFEST_SKELETON.version}`);
