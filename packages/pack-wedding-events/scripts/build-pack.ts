// packages/pack-wedding-events/scripts/build-pack.ts
// T-526 — Build script for `@stageflip/pack-wedding-events`. Walks
// the pack source dir, synthesizes a deterministic SFPACK1 archive
// (per `@stageflip/pack-signing/archive`), signs the archive with the
// supplied Ed25519 private key (per
// `@stageflip/pack-format/signature`), computes the SHA-256 integrity
// hash, materializes the final `manifest.json` with the correct hash,
// and emits `archive.sfpack` + `signature.bin` + `manifest.json` to
// the configured output directory (default
// `packs/stageflip/wedding-events/<MANIFEST_SKELETON.version>/`).
//
// The CLI default outDir is derived from `MANIFEST_SKELETON.version`
// (per the T-510 bug-fix carried into this skeleton) so the default
// output directory tracks the manifest version automatically on each
// minor / patch bump. Operators pinning a non-default outDir continue
// to set `STAGEFLIP_WEDDING_EVENTS_OUT`.
//
...
if (isMainModule) {
  ...
  const keyPath = process.env.STAGEFLIP_WEDDING_EVENTS_KEY
    ? resolve(process.env.STAGEFLIP_WEDDING_EVENTS_KEY)
    : resolve(packageRoot, 'keys/wedding-events-dev.private.pem');
  const sourceDir = process.env.STAGEFLIP_WEDDING_EVENTS_SRC
    ? resolve(process.env.STAGEFLIP_WEDDING_EVENTS_SRC)
    : resolve(packageRoot, 'packs');
  const outDir = process.env.STAGEFLIP_WEDDING_EVENTS_OUT
    ? resolve(process.env.STAGEFLIP_WEDDING_EVENTS_OUT)
    : resolve(packageRoot, `../../packs/stageflip/wedding-events/${MANIFEST_SKELETON.version}`);
