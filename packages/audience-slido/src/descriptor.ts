// packages/audience-slido/src/descriptor.ts
// Static `AdapterDescriptor` for the Slido vendor audience-backend
// adapter (T-479). First of five vendor bridges per ADR-009 §D8.
//
// `id` = `'audience-slido'` is the registry key.
// `license.kind` = `'proprietary-byo'` — Slido is SaaS; tenants supply
// API credentials via TenantAdapterCredentialsStore (T-444).
// `sandbox.kind` = `'remote-service'` — calls Slido's hosted REST API.
//
...
//
// `maxConcurrentVoters: 5000` matches Slido's enterprise-plan cap.
// `snapshotCadenceHz: 5` matches Slido's lower push frequency vs. the
...
/**
 * Env var the production wire-up reads for the Slido API base URL.
 * `sandbox.baseUrlEnvVar` per the `remote-service` schema; stub-mode
 * never reads this.
 */
export const AUDIENCE_SLIDO_BASE_URL_ENV_VAR = 'SLIDO_API_BASE_URL';

/**
 * Per ADR-009 §D8 vendor parity matrix line: `audience-slido | ✓ | ✓ |
 * ✓ | partial[¹] | ✓ | ✓ | ✗ | ✗ | ✗`. Slido leaderboard is tied to
 ...
 */
export const AUDIENCE_SLIDO_SUPPORTED_CLIP_KINDS: readonly AudienceClipKind[] = [
 ...
 */
export const audienceSlidoCapability: AudienceCapabilityDescriptor = {
  persistenceTier: 'durable',
  maxConcurrentVoters: 5000,
  supportedClipKinds: AUDIENCE_SLIDO_SUPPORTED_CLIP_KINDS,
 ...
 *
 * `costPerCall.usd = 0` — Slido billing is tenant-handled out of band
 * via the tenant's own Slido contract. The StageFlip tenant cost
 * tracker (T-443) records adapter invocation counts; dollar
 * attribution to Slido is the tenant's reconciliation.
 *
 * `latencyMs.{p50,p95}` are the vendor's stated latency (Slido
 * typically ~150 / ~400 ms vendor-side; production wire-up T-479a
 * calibrates against real numbers).
 */
export const audienceSlidoDescriptor: AdapterDescriptor = {
  id: 'audience-slido',
  modality: { kind: 'audience-backend' },
  capability: { ...audienceSlidoCapability },
  license: { kind: 'proprietary-byo' },
  sandbox: { kind: 'remote-service', baseUrlEnvVar: AUDIENCE_SLIDO_BASE_URL_ENV_VAR },
  costPerCall: { usd: 0 },
