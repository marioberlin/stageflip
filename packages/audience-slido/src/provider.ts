// packages/audience-slido/src/provider.ts
// `SlidoAudienceProvider` — vendor `AudienceBackendProvider` (T-479)
// wrapping Slido's hosted REST API. Stub-mode v1 per the P14
// reference-adapter convention; production wire-up (Slido REST
// integration) is gated on T-479a (Slido enterprise credentials +
// OAuth audit pending).
//
...
import { audienceSlidoDescriptor } from './descriptor.js';
 ...
 * - `'production'`: real Slido REST API calls. NOT IMPLEMENTED in
 *   T-479 — methods throw `NotYetImplementedError`. Gated on T-479a.
 */
export type SlidoProviderMode = 'stub' | 'production';

/**
 * Thrown by `SlidoAudienceProvider` methods in `'production'` mode
 * until T-479a wires the Slido REST integration.
 */
...
export interface SlidoAudienceProviderOptions {
  readonly mode?: SlidoProviderMode;
  /**
   * Slido API base URL. Defaults to the public hosted endpoint;
...
const DEFAULT_BASE_URL = 'https://api.slido.com/v1';

/**
 * Build the Slido `AudienceBackendProvider`. In `'stub'` mode, methods
 ...
 * integration.
 */
export function createSlidoAudienceProvider(
  options: SlidoAudienceProviderOptions = {},
): AudienceBackendProvider {
  const mode: SlidoProviderMode = options.mode ?? 'stub';
  ...
  async function openSession(call: OpenSessionCall): Promise<SessionHandle> {
    if (unsupportedKind(call.clipKind)) {
      throw new Error(`audience-slido: clipKind '${call.clipKind}' not supported by Slido`);
    }
    if (mode === 'production') {
      throw new NotYetImplementedError('Slido REST integration pending T-479a');
    }
    return {
      sessionId: call.sessionId,
      joinUrl: `https://app.slido.com/join/${call.sessionId}`,
  ...
  async function submitVote(call: SubmitVoteCall): Promise<VoteAck> {
    ...
    return {
      eventId: `slido-evt-${serverTimestamp}-${call.voterToken.slice(0, 6)}`,
      serverTimestamp,
      accepted: true,
    };
  }
  ...
  async function closeSession(call: CloseSessionCall): Promise<FinalSnapshot> {
    ...
    return {
      ...
      voterCount: 0,
      // Slido does not report per-clip-kind aggregation shape to the
  ...
  return {
    ...
    capability:
      audienceSlidoDescriptor.capability as unknown as AudienceBackendProvider['capability'],
    openSession,
    submitVote,
    subscribe,
    closeSession,
