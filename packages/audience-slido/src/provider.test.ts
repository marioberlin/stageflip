// packages/audience-slido/src/provider.test.ts
// Unit tests for createSlidoAudienceProvider (T-479) stub-mode body.

import { describe, expect, it } from 'vitest';

import { audienceSlidoDescriptor } from './descriptor.js';
import { NotYetImplementedError, createSlidoAudienceProvider } from './provider.js';

const FIXED_NOW = Date.parse('2026-05-12T12:00:00.000Z');

function makeProvider() {
  return createSlidoAudienceProvider({ now: () => FIXED_NOW });
}

describe('createSlidoAudienceProvider (stub mode)', () => {
  it('exposes the audienceSlidoDescriptor fields', () => {
    const provider = makeProvider();
    expect(provider.id).toBe(audienceSlidoDescriptor.id);
    expect(provider.modality.kind).toBe('audience-backend');
    expect(provider.capability.supportsMotionNative).toBe(false);
  });

  describe('openSession', () => {
    it('returns a Slido-style SessionHandle for supported clip kinds', async () => {
      const provider = makeProvider();
      const handle = await provider.openSession({
        tenantId: 'tenant-1',
        projectId: 'proj-1',
        sessionId: 'slido-sess-1',
        clipKind: 'live-poll-multiple-choice',
      });
      expect(handle.sessionId).toBe('slido-sess-1');
      expect(handle.joinUrl).toBe('https://app.slido.com/join/slido-sess-1');
      expect(handle.joinCode).toBe('SLIDO-');
    });
    ...
    it('rejects unsupported motion-native clip kinds (heatmap)', async () => {
      ...
      ).rejects.toThrow(/clipKind 'heatmap' not supported by Slido/);
    });
  ...
  describe('production mode', () => {
    it('throws NotYetImplementedError on openSession', async () => {
      const provider = createSlidoAudienceProvider({ mode: 'production' });
      await expect(
        provider.openSession({
          tenantId: 't',
          projectId: 'p',
  ...
  describe('submitVote (stub mode)', () => {
    it('returns an accepted VoteAck with deterministic eventId', async () => {
      ...
      expect(ack.eventId.startsWith('slido-evt-')).toBe(true);
