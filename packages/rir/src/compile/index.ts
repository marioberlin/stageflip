...
function toRIRContent(el: Element): RIRElementContent {
  switch (el.type) {
    ...
    case 'live-poll-open-text':
      // T-462: Second audience-clip element variant. Same lowering as
      // T-461 — `runtime: 'audience'` + `clipName: 'live-poll-open-text'`
      // so `@stageflip/runtimes-audience` `audienceRuntime` picks it up.
      return {
        type: 'clip',
        runtime: 'audience',
        clipName: 'live-poll-open-text',
        params: {
          props: el.props,
          permissions: el.permissions,
          ...(el.provenance !== undefined ? { provenance: el.provenance } : {}),
    ...
    case 'live-poll-rating':
      // T-463: Third audience-clip element variant (closes the LivePoll
      // family). Same lowering as T-461 / T-462 — `runtime: 'audience'` +
      // `clipName: 'live-poll-rating'` so `@stageflip/runtimes-audience`
      // `audienceRuntime` picks it up.
      return {
        type: 'clip',
        runtime: 'audience',
        clipName: 'live-poll-rating',
        params: {
          props: el.props,
          permissions: el.permissions,
          ...(el.provenance !== undefined ? { provenance: el.provenance } : {}),
    ...
    case 'live-quiz':
      // T-465: Fifth audience-clip element variant (competitive
      // multi-question quiz). Same lowering as T-461..T-464 —
      // `runtime: 'audience'` + `clipName: 'live-quiz'` so
    ...
    case 'survey':
      // T-468: Eighth audience-clip element variant (multi-question
      // pre/post survey; closes standard-family v1). Same lowering as
      // T-461..T-467 — `runtime: 'audience'` + `clipName: 'survey'` so
      // `@stageflip/runtimes-audience` `audienceRuntime` picks it up.
      return {
        type: 'clip',
        runtime: 'audience',
