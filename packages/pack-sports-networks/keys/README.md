# `@stageflip/pack-sports-networks` — Development Signing Keys
...
- `sports-networks-dev.public.pem` — public key (committed for reproducibility).
- `sports-networks-dev.private.pem` — private key (committed for reproducibility).

**Both files are intentionally NOT present in this commit.** T-511
ships the package skeleton only; the orchestrator generates the
dev keypair and commits it in a follow-up step, then runs

```sh
pnpm --filter @stageflip/pack-sports-networks build:pack
```

to materialize `archive.sfpack` + `signature.bin` + the final
`manifest.json` at `packs/stageflip/sports-networks/0.1.0/`. The
...
- Used only for local CI roundtrip + `pnpm --filter
  @stageflip/pack-sports-networks test`.
...
1. The pack itself is a skeleton until T-515 — its production payload
...
```sh
# from packages/pack-sports-networks/keys/
pnpm --filter @stageflip/pack-signing run-cli generate-keys --out .
mv stageflip-pack.private.pem sports-networks-dev.private.pem
mv stageflip-pack.public.pem sports-networks-dev.public.pem
```
