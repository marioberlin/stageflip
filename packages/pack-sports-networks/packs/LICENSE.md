# Sports Networks — Commercial License

Copyright (c) 2026 StageFlip

SPDX-License-Identifier: LicenseRef-StageFlip-Commercial-1.0

This pack (Sports Networks, SKU: sports-networks-1y) is licensed under
a commercial subscription. Use of this pack on a StageFlip-compatible
platform requires a valid, paid entitlement issued to the using tenant
by StageFlip or an authorised reseller.

1. **Grant.** Subject to payment of the applicable fees and continued
   compliance with these terms, StageFlip grants the
...
4. **No warranty.** The pack is provided "AS IS", without warranty of
   any kind, express or implied. StageFlip disclaims
   all liability arising from use of the pack to the maximum extent
   permitted by applicable law.

5. **Trademarks.** "StageFlip" and the Sports Networks
   name are trademarks of StageFlip. Nothing in this
   licence grants the tenant any right to use those marks except as
   strictly required to identify the source of the pack.

For licensing inquiries contact: licensing@stageflip.dev
