// packages/pack-sports-networks/scripts/build-pack.test.ts
// T-511 — Drives `buildPack` against a temp-dir source layout using a
...
function createFixture(opts?: { skipPresetsDir?: boolean; withDotFile?: boolean }): Fixture {
  const tmpRoot = mkdtempSync(join(tmpdir(), 'sports-networks-build-'));
  ...
  if (!opts?.skipPresetsDir) {
    ...
    writeFileSync(
      join(presetsDir, 'nba-pro-register-placeholder.md'),
      '---\nid: nba-pro-register-placeholder\n---\n# placeholder\n',
    );
    writeFileSync(
      join(presetsDir, 'nfl-pro-register-placeholder.md'),
      '---\nid: nfl-pro-register-placeholder\n---\n# placeholder\n',
    );
    writeFileSync(
      join(presetsDir, 'mlb-register-placeholder.md'),
      '---\nid: mlb-register-placeholder\n---\n# placeholder\n',
    );
    writeFileSync(
      join(presetsDir, 'f1-pro-register-placeholder.md'),
      '---\nid: f1-pro-register-placeholder\n---\n# placeholder\n',
    );
...
describe('buildPack', () => {
  ...
  it('the manifest.integrity.hash equals SHA-256 over the archive bytes without the manifest', () => {
    ...
    const archiveBytes = synthesizeArchive([
      ...
      {
        path: 'presets/f1-pro-register-placeholder.md',
        content: readFileSync(join(fx.sourceDir, 'presets', 'f1-pro-register-placeholder.md')),
      },
      {
        path: 'presets/mlb-register-placeholder.md',
        content: readFileSync(join(fx.sourceDir, 'presets', 'mlb-register-placeholder.md')),
      },
      {
        path: 'presets/nba-pro-register-placeholder.md',
        content: readFileSync(join(fx.sourceDir, 'presets', 'nba-pro-register-placeholder.md')),
      },
      {
        path: 'presets/nfl-pro-register-placeholder.md',
        content: readFileSync(join(fx.sourceDir, 'presets', 'nfl-pro-register-placeholder.md')),
      },
    ]);
  ...
  it('skips dot-files in the source dir (e.g. .DS_Store)', () => {
    ...
    // archive.
    const otherTmpRoot = mkdtempSync(join(tmpdir(), 'sports-networks-build-noDot-'));
    try {
  ...
  it('the manifest declares the paid-per-tenant license with the sports-networks-1y SKU', () => {
    const result = buildPack({
      sourceDir: fx.sourceDir,
      outDir: fx.outDir,
      privateKeyPem: fx.privateKeyPem,
...
describe('build-pack CLI default outDir (T-511 — version-templated, per T-510 fix)', () => {
  ...
  // expression in the CLI is `resolve(packageRoot, ../../packs/stageflip/sports-networks/${MANIFEST_SKELETON.version})`.
  ...
  it('build-pack.ts CLI source uses ${MANIFEST_SKELETON.version} (NOT a hard-coded 0.1.0 literal) in the default outDir expression', () => {
    ...
    expect(cliSource).toContain(
      '`../../packs/stageflip/sports-networks/${MANIFEST_SKELETON.version}`',
    );
    // Negative: no hard-coded version literal remains in the default outDir.
    expect(cliSource).not.toContain("'../../packs/stageflip/sports-networks/0.1.0'");
    expect(cliSource).not.toContain('"../../packs/stageflip/sports-networks/0.1.0"');
  });

  it('package-relative default outDir resolves under packs/stageflip/sports-networks/<manifest-version>/', () => {
    const here = dirname(fileURLToPath(import.meta.url));
...
describe('buildPack — outDir is honored verbatim regardless of MANIFEST_SKELETON.version (separation-of-concerns seam)', () => {
  ...
  beforeEach(() => {
    tmpRoot = mkdtempSync(join(tmpdir(), 'sports-networks-explicit-outdir-'));
  });
