// packages/pack-sports-networks/scripts/build-pack.ts
// T-511 — Build script for `@stageflip/pack-sports-networks`. Walks
...
// to set `STAGEFLIP_SPORTS_NETWORKS_OUT`.
//
...
if (isMainModule) {
  ...
  const keyPath = process.env.STAGEFLIP_SPORTS_NETWORKS_KEY
    ? resolve(process.env.STAGEFLIP_SPORTS_NETWORKS_KEY)
    : resolve(packageRoot, 'keys/sports-networks-dev.private.pem');
  const sourceDir = process.env.STAGEFLIP_SPORTS_NETWORKS_SRC
    ? resolve(process.env.STAGEFLIP_SPORTS_NETWORKS_SRC)
    : resolve(packageRoot, 'packs');
  const outDir = process.env.STAGEFLIP_SPORTS_NETWORKS_OUT
    ? resolve(process.env.STAGEFLIP_SPORTS_NETWORKS_OUT)
    : resolve(packageRoot, `../../packs/stageflip/sports-networks/${MANIFEST_SKELETON.version}`);
