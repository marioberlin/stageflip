// packages/pack-finance/scripts/build-pack.ts
// T-521 — Build script for `@stageflip/pack-finance`. Walks the pack
...
if (isMainModule) {
  ...
  const keyPath = process.env.STAGEFLIP_FINANCE_KEY
    ? resolve(process.env.STAGEFLIP_FINANCE_KEY)
    : resolve(packageRoot, 'keys/finance-dev.private.pem');
  const sourceDir = process.env.STAGEFLIP_FINANCE_SRC
    ? resolve(process.env.STAGEFLIP_FINANCE_SRC)
    : resolve(packageRoot, 'packs');
  const outDir = process.env.STAGEFLIP_FINANCE_OUT
    ? resolve(process.env.STAGEFLIP_FINANCE_OUT)
    : resolve(packageRoot, `../../packs/stageflip/finance/${MANIFEST_SKELETON.version}`);
