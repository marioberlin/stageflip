# `@stageflip/pack-finance` — Development Signing Keys
...
- `finance-dev.public.pem` — public key (committed for reproducibility).
- `finance-dev.private.pem` — private key (committed for reproducibility).

**Both files are intentionally NOT present in this commit.** T-521
ships the package skeleton only; the orchestrator generates the
dev keypair and commits it in a follow-up step, then runs

```sh
pnpm --filter @stageflip/pack-finance build:pack
```

to materialize `archive.sfpack` + `signature.bin` + the final
`manifest.json` at `packs/stageflip/finance/0.1.0/`. The
...
1. The pack itself is a skeleton until T-525 — its production payload
...
```sh
# from packages/pack-finance/keys/
pnpm --filter @stageflip/pack-signing run-cli generate-keys --out .
mv stageflip-pack.private.pem finance-dev.private.pem
mv stageflip-pack.public.pem finance-dev.public.pem
```
