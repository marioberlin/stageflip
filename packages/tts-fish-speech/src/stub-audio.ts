// packages/tts-fish-speech/src/stub-audio.ts
// Deterministic silent-WAV generator for the stub-mode
// FishSpeechTtsProvider. Pure function: same (durationS, sampleRate) →
// byte-identical output. No `Date`, no `Math.random`, no `fetch`, no
// timers.
//
// Output is a `data:audio/wav;base64,...` URI for a mono 16-bit PCM WAV
// with every PCM sample set to zero (silent). The WAV header follows
// the standard RIFF/WAVE/fmt /data 44-byte layout. Production wire-up
// (T-427a) replaces this with a real Fish Speech library call (Python
// sidecar over a pinned model checkpoint); the surrounding shape (URL
// string) stays identical.
//
// Identical implementation to `@stageflip/tts-kokoro`'s
// `generateSilentWavDataUri` — both adapters synthesize silent WAV
// payloads at their native sample rate (Kokoro 24000Hz; Fish Speech
// 22050Hz). Kept independent here (no cross-package dependency)
// because the function is small and pure, and a runtime dependency
// between adapters would couple their release cadence.

/**
 * Generate a deterministic silent-WAV `data:` URI.
 *
 * Layout: 44-byte standard RIFF/WAVE header (mono, 16-bit PCM)
 * followed by `floor(durationS * sampleRate) * 2` zero bytes for the
 * samples.
 *
...
export function generateSilentWavDataUri(durationS: number, sampleRate: number): string {
  ...
  // PCM samples (offset 44..end) are already zero — Uint8Array
  // initializes to zero. No further work needed.
...
/**
 * Encode a `Uint8Array` to a base64 string. Uses `Buffer` when running
 * on Node; falls back to `btoa` otherwise. Both paths are
 * deterministic + synchronous; tests cover the Node path.
 */
function bytesToBase64(bytes: Uint8Array): string {
  // Node path — `Buffer` is the fastest option on every supported
  // runtime.
