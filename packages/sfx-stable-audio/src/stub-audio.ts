// packages/sfx-stable-audio/src/stub-audio.ts
// Deterministic silent-WAV generator for the stub-mode
// StableAudioSfxProvider. Pure function: same (durationS, sampleRate) →
...
// (T-434a) replaces this with a real Stable Audio Open call; the
// surrounding shape (URL string) stays identical.
//
// Mirrors `packages/music-acestep/src/stub-audio.ts` +
// `packages/music-yue/src/stub-audio.ts` verbatim — the silent-WAV
// emitter is sample-rate agnostic, so the same helper services both
// music adapters' 44100Hz default and this SFX adapter's 44100Hz
// default. Duplication is intentional: each adapter package owns its
// stub-audio module to keep `pnpm check-asset-licenses`'s adapter
// discovery walker self-contained (no cross-adapter imports).
