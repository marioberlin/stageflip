// packages/pack-frontier-fx/scripts/build-pack.ts
// T-531 — Build script for `@stageflip/pack-frontier-fx`. Walks
...
// to set `STAGEFLIP_FRONTIER_FX_OUT`.
//
...
if (isMainModule) {
  ...
  const keyPath = process.env.STAGEFLIP_FRONTIER_FX_KEY
    ? resolve(process.env.STAGEFLIP_FRONTIER_FX_KEY)
    : resolve(packageRoot, 'keys/frontier-fx-dev.private.pem');
  const sourceDir = process.env.STAGEFLIP_FRONTIER_FX_SRC
    ? resolve(process.env.STAGEFLIP_FRONTIER_FX_SRC)
    : resolve(packageRoot, 'packs');
  const outDir = process.env.STAGEFLIP_FRONTIER_FX_OUT
    ? resolve(process.env.STAGEFLIP_FRONTIER_FX_OUT)
    : resolve(packageRoot, `../../packs/stageflip/frontier-fx/${MANIFEST_SKELETON.version}`);
