// packages/tts-kokoro/src/stub-audio.test.ts
...
describe('generateSilentWavDataUri', () => {
  it('returns a data:audio/wav;base64, URI', () => {
    const uri = generateSilentWavDataUri(1, 24000);
    expect(uri.startsWith('data:audio/wav;base64,')).toBe(true);
  });

  it('emits the RIFF/WAVE/fmt magics in the header', () => {
    const bytes = decodeDataUri(generateSilentWavDataUri(1, 24000));
  ...
  it('writes the data-chunk size = floor(durationS * sampleRate) * 2 (16-bit mono)', () => {
    const bytes = decodeDataUri(generateSilentWavDataUri(1.5, 24000));
    const expectedBytes = Math.floor(1.5 * 24000) * 2;
    expect(readUint32LE(bytes, 40)).toBe(expectedBytes);
  });

  it('emits zero PCM samples (all silent)', () => {
    const bytes = decodeDataUri(generateSilentWavDataUri(0.25, 24000));
  ...
  it('produces different bytes for different durations', () => {
    const a = generateSilentWavDataUri(1, 24000);
    const b = generateSilentWavDataUri(2, 24000);
    expect(a).not.toBe(b);
  });

  it('produces different bytes for different sample rates', () => {
    const a = generateSilentWavDataUri(1, 24000);
    const b = generateSilentWavDataUri(1, 48000);
    expect(a).not.toBe(b);
  });

  it('rejects non-positive durationS', () => {
    expect(() => generateSilentWavDataUri(0, 24000)).toThrow(/positive finite/);
    expect(() => generateSilentWavDataUri(-1, 24000)).toThrow(/positive finite/);
    expect(() => generateSilentWavDataUri(Number.NaN, 24000)).toThrow(/positive finite/);
    expect(() => generateSilentWavDataUri(Number.POSITIVE_INFINITY, 24000)).toThrow(
      /positive finite/,
    );
  });
  ...
  it('writes a 44-byte header followed by data-chunk-size sample bytes', () => {
    const bytes = decodeDataUri(generateSilentWavDataUri(0.5, 24000));
