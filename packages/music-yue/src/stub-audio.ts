// packages/music-yue/src/stub-audio.ts
// Deterministic silent-WAV generator for the stub-mode YueMusicProvider.
...
// (T-433a) replaces this with a real YuE ONNX call; the surrounding
// shape (URL string) stays identical.
//
// Mirrors `packages/music-acestep/src/stub-audio.ts` verbatim — the
// silent-WAV emitter is sample-rate agnostic. Duplication is
// intentional: each adapter package owns its stub-audio module to
// keep `pnpm check-asset-licenses`'s adapter discovery walker
// self-contained (no cross-adapter imports).
