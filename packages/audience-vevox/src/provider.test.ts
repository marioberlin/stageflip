// packages/audience-vevox/src/provider.test.ts
// Unit tests for createVevoxAudienceProvider (T-479) stub-mode body.

import { describe, expect, it } from 'vitest';

import { audienceVevoxDescriptor } from './descriptor.js';
import { NotYetImplementedError, createVevoxAudienceProvider } from './provider.js';

const FIXED_NOW = Date.parse('2026-05-12T12:00:00.000Z');

function makeProvider() {
  return createVevoxAudienceProvider({ now: () => FIXED_NOW });
}

describe('createVevoxAudienceProvider (stub mode)', () => {
  it('exposes the audienceVevoxDescriptor fields', () => {
    const provider = makeProvider();
    expect(provider.id).toBe(audienceVevoxDescriptor.id);
    expect(provider.modality.kind).toBe('audience-backend');
    expect(provider.capability.supportsMotionNative).toBe(false);
  });

  describe('openSession', () => {
    it('returns a Vevox-style SessionHandle for supported clip kinds', async () => {
      const provider = makeProvider();
      const handle = await provider.openSession({
        tenantId: 'tenant-1',
        projectId: 'proj-1',
        sessionId: 'vevox-sess-1',
        clipKind: 'live-poll-multiple-choice',
      });
      expect(handle.sessionId).toBe('vevox-sess-1');
      expect(handle.joinUrl).toBe('https://app.vevox.com/join/vevox-sess-1');
      expect(handle.joinCode).toBe('VEVOX-');
    });
    ...
    it('rejects unsupported motion-native clip kinds (heatmap)', async () => {
      ...
      ).rejects.toThrow(/clipKind 'heatmap' not supported by Vevox/);
    });
  ...
  describe('production mode', () => {
    it('throws NotYetImplementedError on openSession', async () => {
      const provider = createVevoxAudienceProvider({ mode: 'production' });
      await expect(
        provider.openSession({
          tenantId: 't',
          projectId: 'p',
  ...
  describe('submitVote (stub mode)', () => {
    it('returns an accepted VoteAck with deterministic eventId', async () => {
      ...
      expect(ack.eventId.startsWith('vevox-evt-')).toBe(true);
