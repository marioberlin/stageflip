// packages/runtimes/audience/src/clips/live-poll-rating/factory.test.ts
// T-463 — Factory tests for `livePollRatingClipFactory`. Verifies:
//  - empty-live-mount route renders the empty-state.
...
function makeCtx(opts: {
  ...
  const ctx = {
    clip: {
      props: { question: 'Rate the talk', scaleMin: 1, scaleMax: 5 },
...
describe('livePollRatingClipFactory', () => {
  ...
  it('rejects when props are malformed', async () => {
    ...
    const ctx = {
      clip: {
        props: { question: '', scaleMin: 1, scaleMax: 1 },
    ...
    await expect(livePollRatingClipFactory(ctx)).rejects.toThrow(/invalid props/);
    controller.abort();
  });
