// packages/runtimes/audience/src/clips/live-poll-rating/factory.ts
// T-463 — `AudienceClipFactory` for the `live-poll-rating` clip.
...
//   - `empty-live-mount`  → render the empty-state scaffold (zero counts).
//
...
function paint(
  ...
  if (props.labels !== undefined) ctx.labels = props.labels;
  const tree = renderLivePollRatingStaticFallback({
 ...
 * react-dom/client.
 */
export const livePollRatingClipFactory: AudienceClipFactory = async (
  ...
  const propsResult = livePollRatingClipPropsSchema.safeParse(rawProps);
  if (!propsResult.success) {
    throw new Error(`livePollRatingClipFactory: invalid props — ${propsResult.error.message}`);
  }
  ...
  const initialAggregation: LivePollRatingAggregation = (() => {
    if (route === 'staticFallback' && ctx.provenance !== undefined) {
      const inner = ctx.provenance.aggregation;
      if (inner.kind === 'live-poll-rating') return inner;
    }
  ...
  if (route === 'live' && ctx.sessionId !== undefined) {
    ...
    if (authToken !== undefined) {
      const onSnapshot = (snapshot: AggregationSnapshot): void => {
        ...
        if (inner.kind !== 'live-poll-rating') return;
