// packages/runtimes/audience/src/clips/live-poll-multiple-choice/factory.ts
// T-461 — `AudienceClipFactory` for the `live-poll-multiple-choice`
// clip. Three-state router (ADR-010 §D8) decides the mount path:
...
//   - `empty-live-mount`  → render the empty-state scaffold (zero bars).
//
...
function paint(
  state: MountState,
  props: LivePollMultipleChoiceClipProps,
  width: number,
  height: number,
): void {
  if (state.disposed) return;
  const tree = renderLivePollMultipleChoiceStaticFallback({
 ...
 * react-dom/client.
 */
export const livePollMultipleChoiceClipFactory: AudienceClipFactory = async (
  ...
  const propsResult = livePollMultipleChoiceClipPropsSchema.safeParse(rawProps);
  if (!propsResult.success) {
    throw new Error(
      `livePollMultipleChoiceClipFactory: invalid props — ${propsResult.error.message}`,
    );
  }
  ...
  const initialAggregation: LivePollMultipleChoiceAggregation = (() => {
    if (route === 'staticFallback' && ctx.provenance !== undefined) {
      const inner = ctx.provenance.aggregation;
      if (inner.kind === 'live-poll-multiple-choice') return inner;
    }
    return {
  ...
  if (route === 'live' && ctx.sessionId !== undefined) {
    ...
    if (authToken !== undefined) {
      const onSnapshot = (snapshot: AggregationSnapshot): void => {
        ...
        if (inner.kind !== 'live-poll-multiple-choice') return;
