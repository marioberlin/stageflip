// packages/runtimes/audience/src/clips/leaderboard/factory.ts
// T-466 — `AudienceClipFactory` for the `leaderboard` clip.
...
//   - `empty-live-mount`  → render the "Waiting for participants…"
//                           placeholder (no ranking entries yet).
//
...
//
// Note: `LeaderboardVote = never` per ADR-010 §D2 — this factory only
// SUBSCRIBES to the derived aggregation, it never emits a submitVote.
// The voter UI for this kind is the `<LeaderboardViewOnly>` notice
// in `apps/audience-join/src/components/leaderboard-view-only.tsx`.
...
function paint(
  state: MountState,
  props: LeaderboardClipProps,
  width: number,
  height: number,
): void {
  if (state.disposed) return;
  const ctx: { width: number; height: number; title?: string } = {
    width,
    height,
    ...(props.title !== undefined ? { title: props.title } : {}),
  };
  const tree = renderLeaderboardStaticFallback({
 ...
 * react-dom/client.
 */
export const leaderboardClipFactory: AudienceClipFactory = async (
  ...
  const propsResult = leaderboardClipPropsSchema.safeParse(rawProps);
  if (!propsResult.success) {
    throw new Error(`leaderboardClipFactory: invalid props — ${propsResult.error.message}`);
  }
  ...
  if (route === 'live' && ctx.sessionId !== undefined) {
    ...
    if (authToken !== undefined) {
      const onSnapshot = (snapshot: AggregationSnapshot): void => {
        ...
        if (inner.kind !== 'leaderboard') return;
