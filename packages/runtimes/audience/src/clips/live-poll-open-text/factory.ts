// packages/runtimes/audience/src/clips/live-poll-open-text/factory.ts
// T-462 — `AudienceClipFactory` for the `live-poll-open-text` clip.
...
//   - `empty-live-mount`  → render the empty-state scaffold (zero rows).
//
...
function paint(
  state: MountState,
  props: LivePollOpenTextClipProps,
  width: number,
  height: number,
): void {
  if (state.disposed) return;
  const tree = renderLivePollOpenTextStaticFallback({
 ...
 * react-dom/client.
 */
export const livePollOpenTextClipFactory: AudienceClipFactory = async (
  ...
  const propsResult = livePollOpenTextClipPropsSchema.safeParse(rawProps);
  if (!propsResult.success) {
    throw new Error(`livePollOpenTextClipFactory: invalid props — ${propsResult.error.message}`);
  }
  ...
  const initialAggregation: LivePollOpenTextAggregation = (() => {
    if (route === 'staticFallback' && ctx.provenance !== undefined) {
      const inner = ctx.provenance.aggregation;
      if (inner.kind === 'live-poll-open-text') return inner;
    }
    return {
  ...
  if (route === 'live' && ctx.sessionId !== undefined) {
    ...
    if (authToken !== undefined) {
      const onSnapshot = (snapshot: AggregationSnapshot): void => {
        ...
        if (inner.kind !== 'live-poll-open-text') return;
