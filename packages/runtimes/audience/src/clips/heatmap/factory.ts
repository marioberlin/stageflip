// packages/runtimes/audience/src/clips/heatmap/factory.ts
// T-469 — `AudienceClipFactory` for the `heatmap` clip.
...
//   - `empty-live-mount`  → render the "Waiting for taps…" placeholder
...
// The static-fallback render + the `rasterizeHeatmap` step are pure.
 ...
 */
function paint(state: MountState, props: HeatmapClipProps, width: number, height: number): void {
  if (state.disposed) return;
  const tree = renderHeatmapStaticFallback({
 ...
 * react-dom/client.
 */
export const heatmapClipFactory: AudienceClipFactory = async (
  ...
  const propsResult = heatmapClipPropsSchema.safeParse(rawProps);
  if (!propsResult.success) {
    throw new Error(`heatmapClipFactory: invalid props — ${propsResult.error.message}`);
  }
