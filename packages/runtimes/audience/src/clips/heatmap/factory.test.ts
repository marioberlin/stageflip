// packages/runtimes/audience/src/clips/heatmap/factory.test.ts
// T-469 — Factory tests for `heatmapClipFactory`. Verifies:
...
describe('heatmapClipFactory', () => {
  ...
  it('renders staticFallback from provenance.aggregation when sessionId is absent', async () => {
    ...
    const ctx = makeCtx({
      ...
      provenance: {
        ...
        aggregation: {
          kind: 'heatmap',
          taps: [
            { x: 0.5, y: 0.5, intensity: 3 },
            { x: 0.3, y: 0.4, intensity: 2 },
          ],
          totalTaps: 5,
          gridResolution: { w: 32, h: 32 },
        },
    ...
    expect(total?.getAttribute('data-total-taps')).toBe('5');
    handle.dispose();
    controller.abort();
  });
  ...
  it('opens a subscribe loop on the live route and re-renders on snapshots', async () => {
    ...
    expect(root?.getAttribute('data-state')).toBe('aggregated');
    const total = document.querySelector('[data-testid="heatmap-total"]');
    expect(total?.getAttribute('data-total-taps')).toBe('1');
    handle.dispose();
    close();
    controller.abort();
  });
  ...
  it('rejects when props are malformed', async () => {
    ...
    await expect(heatmapClipFactory(ctx)).rejects.toThrow(/invalid props/);
    controller.abort();
  });
