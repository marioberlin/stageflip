// packages/runtimes/audience/src/clips/audience-ai-prompt/factory.ts
// T-471 — `AudienceClipFactory` for the `audience-ai-prompt` clip.
...
//   - `empty-live-mount`  → render the empty voting state (no prompts).
//
...
interface MountState {
  reactRoot: Root;
  aggregation: AudienceAiPromptAggregation;
  disposed: boolean;
}
...
function paint(
  state: MountState,
  props: AudienceAiPromptClipProps,
  width: number,
  height: number,
): void {
  if (state.disposed) return;
  const tree = renderAudienceAiPromptStaticFallback({
 ...
 * react-dom/client.
 */
export const audienceAiPromptClipFactory: AudienceClipFactory = async (
  ...
  const propsResult = audienceAiPromptClipPropsSchema.safeParse(rawProps);
  if (!propsResult.success) {
    throw new Error(`audienceAiPromptClipFactory: invalid props — ${propsResult.error.message}`);
  }
  ...
  const initialAggregation: AudienceAiPromptAggregation = (() => {
    if (route === 'staticFallback' && ctx.provenance !== undefined) {
      const inner = ctx.provenance.aggregation;
      if (inner.kind === 'audience-ai-prompt') return inner;
    }
    return {
  ...
  if (route === 'live' && ctx.sessionId !== undefined) {
    ...
    if (authToken !== undefined) {
      const onSnapshot = (snapshot: AggregationSnapshot): void => {
        ...
        if (inner.kind !== 'audience-ai-prompt') return;
