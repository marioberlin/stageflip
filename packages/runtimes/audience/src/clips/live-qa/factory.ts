// packages/runtimes/audience/src/clips/live-qa/factory.ts
// T-464 — `AudienceClipFactory` for the `live-qa` clip.
...
//   - `empty-live-mount`  → render the empty-state scaffold (zero
//                           questions).
//
 ...
 */
function paint(state: MountState, props: LiveQAClipProps, width: number, height: number): void {
  ...
  const tree = renderLiveQAStaticFallback({
 ...
 * react-dom/client.
 */
export const liveQAClipFactory: AudienceClipFactory = async (
  ...
  const propsResult = liveQAClipPropsSchema.safeParse(rawProps);
  if (!propsResult.success) {
    throw new Error(`liveQAClipFactory: invalid props — ${propsResult.error.message}`);
  }
