// packages/runtimes/audience/src/clips/word-cloud/factory.ts
// T-467 — `AudienceClipFactory` for the `word-cloud` clip.
...
//   - `empty-live-mount`  → render the "Waiting for submissions…"
//                           placeholder (no word entries yet).
//
...
interface MountState {
  reactRoot: Root;
  aggregation: WordCloudAggregation;
  disposed: boolean;
}
 ...
 */
function paint(state: MountState, props: WordCloudClipProps, width: number, height: number): void {
  if (state.disposed) return;
  const ctx: { width: number; height: number; prompt?: string } = {
    width,
    height,
    prompt: props.prompt,
  };
  const tree = renderWordCloudStaticFallback({
 ...
 * react-dom/client.
 */
export const wordCloudClipFactory: AudienceClipFactory = async (
  ...
  const propsResult = wordCloudClipPropsSchema.safeParse(rawProps);
  if (!propsResult.success) {
    throw new Error(`wordCloudClipFactory: invalid props — ${propsResult.error.message}`);
  }
  ...
  if (route === 'live' && ctx.sessionId !== undefined) {
    ...
    if (authToken !== undefined) {
      const onSnapshot = (snapshot: AggregationSnapshot): void => {
        ...
        if (inner.kind !== 'word-cloud') return;
