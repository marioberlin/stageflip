// packages/runtimes/audience/src/clips/survey/factory.ts
// T-468 — `AudienceClipFactory` for the `survey` clip.
...
//   - `empty-live-mount`  → render the "Waiting for responses…"
//                           placeholder (no question aggregations yet).
//
 ...
 */
function paint(state: MountState, props: SurveyClipProps, width: number, height: number): void {
  if (state.disposed) return;
  const tree = renderSurveyStaticFallback({
    snapshot: state.aggregation,
    context: { width, height, questions: props.questions },
  });
 ...
 * react-dom/client.
 */
export const surveyClipFactory: AudienceClipFactory = async (
  ...
  const propsResult = surveyClipPropsSchema.safeParse(rawProps);
  if (!propsResult.success) {
    throw new Error(`surveyClipFactory: invalid props — ${propsResult.error.message}`);
  }
