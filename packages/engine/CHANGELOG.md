...
- 55248cd: T-340 — Add the `cluster-b-compose` engine handler bundle: 4 read-only composer tools (`compose_sports_score` / `compose_standings_table` / `compose_var_call` / `compose_player_intro`) that bind a semantic Cluster B (Sports) brief to a ratified preset id + opaque props payload. Tools declare `ToolContext` (no document reads, no patch sink); the caller mounts the chosen clip via a separate write-tier tool (`add_clip` / `add_element`).
