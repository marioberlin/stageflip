// packages/audience-polleverywhere/src/provider.test.ts
// Unit tests for createPollEverywhereAudienceProvider (T-479) stub-mode body.

import { describe, expect, it } from 'vitest';

import { audiencePollEverywhereDescriptor } from './descriptor.js';
import { NotYetImplementedError, createPollEverywhereAudienceProvider } from './provider.js';

const FIXED_NOW = Date.parse('2026-05-12T12:00:00.000Z');

function makeProvider() {
  return createPollEverywhereAudienceProvider({ now: () => FIXED_NOW });
}

describe('createPollEverywhereAudienceProvider (stub mode)', () => {
  it('exposes the audiencePollEverywhereDescriptor fields', () => {
    const provider = makeProvider();
    expect(provider.id).toBe(audiencePollEverywhereDescriptor.id);
    expect(provider.modality.kind).toBe('audience-backend');
    expect(provider.capability.supportsMotionNative).toBe(false);
  });

  describe('openSession', () => {
    it('returns a PollEverywhere-style SessionHandle for supported clip kinds', async () => {
      const provider = makeProvider();
      const handle = await provider.openSession({
        tenantId: 'tenant-1',
        projectId: 'proj-1',
        sessionId: 'polleverywhere-sess-1',
        clipKind: 'live-poll-multiple-choice',
      });
      expect(handle.sessionId).toBe('polleverywhere-sess-1');
      expect(handle.joinUrl).toBe('https://app.polleverywhere.com/join/polleverywhere-sess-1');
      expect(handle.joinCode).toBe('POLLEV');
    });
    ...
    it('rejects unsupported motion-native clip kinds (heatmap)', async () => {
      ...
      ).rejects.toThrow(/clipKind 'heatmap' not supported by PollEverywhere/);
    });
  ...
  describe('production mode', () => {
    it('throws NotYetImplementedError on openSession', async () => {
      const provider = createPollEverywhereAudienceProvider({ mode: 'production' });
      await expect(
        provider.openSession({
          tenantId: 't',
          projectId: 'p',
  ...
  describe('submitVote (stub mode)', () => {
    it('returns an accepted VoteAck with deterministic eventId', async () => {
      ...
      expect(ack.eventId.startsWith('polleverywhere-evt-')).toBe(true);
