...
import { CallableError, acceptInviteHandler, changeMemberRoleHandler, createApiKeyHandler, inviteMemberHandler, removeMemberHandler, revokeApiKeyHandler, setActiveOrgHandler, } from './auth/index.js';
import { createAdminBackupDeps } from './backup/admin-deps.js';
import { backupFirestoreHandler, backupStorageHandler, verifyBackupHandler, } from './backup/index.js';
import { createAdminBakeDeps } from './bake/admin-deps.js';
import { submitBakeJobAdapter } from './bake/index.js';
initializeApp();
function deps() {
    const env = (process.env.STAGEFLIP_ENV ?? 'dev');
    return {
...
export const setActiveOrg = onCall(async (req) => {
    try {
...
export const backupFirestore = onSchedule({ schedule: '0 2 * * *', timeZone: 'Etc/UTC', timeoutSeconds: 540, memory: '512MiB' }, async () => {
    await backupFirestoreHandler(backupDeps());
});
/** T-272 AC #2: daily Storage backup at 02:00 UTC. */
export const backupStorage = onSchedule({ schedule: '0 2 * * *', timeZone: 'Etc/UTC', timeoutSeconds: 540, memory: '512MiB' }, async () => {
    await backupStorageHandler(backupDeps());
});
/** T-272 AC #5: daily verification at 03:00 UTC (1h after backup). */
export const verifyBackup = onSchedule({ schedule: '0 3 * * *', timeZone: 'Etc/UTC', timeoutSeconds: 300, memory: '256MiB' }, async () => {
    await verifyBackupHandler(backupDeps());
});
// --- T-265: bake submission callable ---
let cachedBakeDeps = null;
function bakeDeps() {
...
export const submitBakeJob = onCall(async (req) => {
    try {
