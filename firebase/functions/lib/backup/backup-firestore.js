...
import { firestoreBackupPrefix, isoDate } from './types.js';
/**
 ...
 */
export async function backupFirestoreHandler(deps) {
    ...
    for (const target of deps.firestoreTargets) {
        ...
        try {
            ...
            deps.logger.error(err instanceof Error ? err : new Error(message), `firestore export failed for ${target.databaseId}`);
        }
    }
    return {
...
//# sourceMappingURL=backup-firestore.js.map