 ...
 */
export async function verifyBackupHandler(deps, options = {}) {
    ...
    if (failures.length === 0) {
        deps.logger.info({ operation: 'verifyBackup', isoDate: date, checked: expected.length }, 'backup verification passed');
    }
    return {
...
function reportFailure(deps, failure, date) {
