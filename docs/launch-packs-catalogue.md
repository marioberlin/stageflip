...
| `wedding-events` | `stageflip` | `0.2.0` | `wedding-events-1y` | `paid-per-tenant` (commercial-subscription) | `cluster-wedding-events` | v0.2.0 GA — skeleton T-526; theme variants rustic / modern / classic T-527; wedding-ceremony + wedding-reception composition templates T-528; transitions + bumpers (petal-cross-fade + lace-wipe + wedding-bumper-card + wedding-final-card) T-529; pre-licensed audio bed library (five instrumental tracks via contributes.assets) T-530 (closes the pack). |
| `frontier-fx` | `stageflip` | `0.1.0` | `frontier-fx-1y` | `paid-per-tenant` (commercial-subscription) | `cluster-i` | Skeleton landed in T-531 with four placeholder presets (`premium-shaders-placeholder`, `3d-asset-library-placeholder`, `reactionstream-physics-placeholder`, `titlesequence-premium-placeholder`); premium shaders T-532; pre-licensed commercial-OK 3D asset library T-533; premium ReactionStream particle physics presets T-534; premium TitleSequence templates T-535 (closes the pack). |
