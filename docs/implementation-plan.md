...
| **T-131e.1 [new]** | **Video / image tranche — 2 clips**: `video-background` + `gif-player`. Both port against the new `<FrameVideo>` / `<FrameImage>` surface from T-131e.0. Parity fixtures + KNOWN_KINDS extension included | M |
| **T-131e.2 [new]** | **Audio tranche — 2 clips**: `voiceover-narration` (reference is text+SVG-only — no `<Audio>` import — so bridge-tier-eligible as-ported; label kept under the T-131e umbrella because production pairs it with an audio track) + `audio-visualizer-reactive` (needs `<FrameAudio>` + a `useWindowedAudioData` analogue for frame-windowed audio samples) | M |
...
| **T-131f.2 [planned]** | **Dashboard composites — 5 clips**: sales-dashboard, product-dashboard, okr-dashboard, hr-dashboard, marketing-dashboard. Each composes existing `AnimatedValue` / `AnimatedProgressBar` / `AnimatedProgressRing` primitives from T-131b.3. Lighter per-clip than the financial-statement family (T-131f.3) | M |
| **T-131f.3 [planned]** | **Financial statement composite**: financial-statement clip + 3 supporting subcomponents (StatementKpiStrip, StatementTable, StatementCommentsRail). Largest single port in T-131 family at ~640 LOC source across 4 files | M |
| **T-131f.4 [deferred]** | **`audio-visualizer` real-audio reactive variant** — separate follow-up. Needs a non-Remotion `<Audio>` wrapper backed by `useMediaSync` from `@stageflip/frame-runtime`, plus the `useWindowedAudioData` analogue from `@remotion/media-utils`. Coordinate with T-131e (which also includes `audio-visualizer-reactive` in its scope) | M |
