# StageFlip — Implementation Plan v1.17
...
114 tasks across three parallel tracks. Reference ADRs: ADR-003 (interactive runtime tier), ADR-004 (preset system), ADR-005 (frontier clip catalogue). PR template: `.github/pr-templates/phase-13.md`.

**Hard gate**: T-301 / T-302 / T-303 (the three ADRs) must merge before any code lands on T-304+. After ADRs merge, three tracks (A: frontier runtime, B: preset library, C: supporting plumbing) run in parallel.
