...
| **T-131b.2 [new]** | **Medium tranche — 5 clips**: subtitle-overlay, light-leak, pie-chart-build, stock-ticker, line-chart-draw. SVG-heavy; some use word-stagger or per-segment math. Extends `ALL_BRIDGE_CLIPS` | M |
| **T-131b.3 [new]** | **Heavy tranche — 4 clips**: pull-quote, comparison-table, kpi-grid, animated-value. The biggest of the bridge ports (~200 LOC source each) — each uses `spring` + multi-stage stagger | M |
| **T-131c [new]** | **`runtimes-gsap` tier**. GSAP-dependent clips identified at port time (read SM source for the actual list). Each registered + parity fixture | M |
| **T-131d [new]** | **`runtimes-lottie` / `runtimes-three` tiers — 5 clips**: lottie-player, scene-3d, shader-bg, particles, animated-map. Heaviest per-clip; declared themeSlots where palette-driven | M |
