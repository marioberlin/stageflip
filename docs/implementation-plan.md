...
| **T-245 [rev]** | **Slide rasterization primitive** — promoted from "crop unsupported shapes only" to `rasterizeFromThumbnail(pageImage, bboxPx, options) => Asset` consumed by both import (image-fallback when an element can't be modeled cleanly: gradient fills, theme effects, autofit-scaled text, low OCR confidence) and the future export-google-slides image-fallback path (T-252). Lives in `@stageflip/import-google-slides` or a new thin shared package | M |
