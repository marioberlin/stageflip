...
| **T-131f.2 [in-progress]** | **Dashboard composites — 5 clips** chosen Option B (flat-prop interface per clip; no `@slidemotion/schema` domain types reimplemented). Landing in sub-tranches — **f.2a** ships `hr-dashboard` + `marketing-dashboard` (simplest standalones, `ALL_BRIDGE_CLIPS` → 26 clips); **f.2b** follows with `product-dashboard` + `okr-dashboard`; **f.2c** with `sales-dashboard` (has a composite pipeline + forecast section, heaviest). Each clip declares its own Zod `propsSchema` over the fields it actually renders — sufficient for the Phase 6 editor surface, the Phase 7 agent layer can map domain schemas to these props later | M |
