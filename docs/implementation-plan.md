# StageFlip — Implementation Plan v1.24
...
**Status**: ✅ **Ratified 2026-04-25** — all 12 tasks merged (T-220 → T-231). Closeout handover at `docs/handover-phase10-complete.md`. Ratification carried by the first P11 PR (T-240 — `@stageflip/import-pptx`).
...
| **T-243c [new]** | **PPTX font asset extraction** — `<p:embeddedFont>` parsing + bytes resolution; new `LF-PPTX-UNRESOLVED-FONT` code | M |
| **T-244 [rev]** | **`@stageflip/import-google-slides` — OAuth + Slides API v1 + native grouping + render-PNG pixel matching**. Parser fetches `presentations.get` plus the per-slide rendered PNG (Drive thumbnail) at fixed render resolution; converts API affine transforms to render-pixel boxes (`emu_to_px = render_size / slide_size`) composing parent-group transforms into nested elements. Per-slide pixel-candidate pass — PaddleOCR (text-line/word polygons + recognized text), OpenCV `findContours` + `connectedComponentsWithStats` (hard-edged shapes), optional SAM 2 (logos / icons / illustrations) — runs cropped to each API element's bbox plus a padding buffer; candidates match to `objectId`s via text-content equality + center-inside containment + z-order plausibility. Emits `GroupElement` for `elementGroup` (recursive, no flattening), `TableElement` for tables (after merged-cell audit in T-251), and captures placeholder inheritance via `inheritsFrom: { templateId, placeholderIdx }` (depends on T-251). New loss flags: `LF-GSLIDES-PADDING-INFERRED`, `LF-GSLIDES-FONT-SUBSTITUTED`, `LF-GSLIDES-IMAGE-FALLBACK`, `LF-GSLIDES-LOW-MATCH-CONFIDENCE`, `LF-GSLIDES-PLACEHOLDER-INLINED`, `LF-GSLIDES-TABLE-MERGE-LOST`. Gemini multimodal fallback (T-246) resolves only regions that failed deterministic matching. License-vet PaddleOCR / OpenCV / SAM 2 (all Apache-2.0) via `pnpm check-licenses` | L |
| **T-244-cv-worker [new]** | **Python CV sidecar (PaddleOCR + OpenCV + SAM 2) for the Slides import pipeline** — production implementation of the `CvCandidateProvider` HTTP contract that T-244's `HttpCvProvider` calls. Infrastructure-only: FastAPI service in `services/cv-worker/`, Cloud Run deploy script, runbook. T-244 ships the TS-side stub provider for tests; this stands up the production server. SAM 2 opt-in via env var. Spec at `docs/tasks/T-244-cv-worker.md` | S |
| **T-245 [rev]** | **Slide rasterization primitive** — `rasterizeFromThumbnail(pageImage, bboxPx, options) => RasterizedAsset` in a new shared package `@stageflip/rasterize`. Pure-TS PNG-crop primitive (via `pngjs`, MIT, already in workspace) with deterministic output, content-hashed asset id, padding clamp, and no loss-flag / `AssetStorage` knowledge — consumers (T-244 import image-fallback, T-252 export image-fallback) wire upload + flag emission at their layer. Spec at `docs/tasks/T-245.md` | M |
| **T-246 [rev]** | **AI-QC convergence loop — inverted: classical CV first, Gemini second**. ~90% of objects are expected to resolve in the cheap deterministic pass (PaddleOCR text-content match, OpenCV / SAM 2 mask containment, z-order plausibility — see T-244). Only regions that fail deterministic matching are sent to Gemini as a per-element prompt (cropped PNG + API metadata + question). Loop iterates until every element has a confidence above threshold or `LF-GSLIDES-LOW-MATCH-CONFIDENCE` is emitted for the residual. Bounds Gemini cost and yields a meaningful per-element confidence on every imported deck | L |
...
| **T-250 [rev]** | **`skills/stageflip/workflows/import-*/SKILL.md` + new export workflow skills** — extended scope: documents API-geometry → render-pixel conversion math; "rendered PNG is the source of truth" principle; pixel-candidate pipeline (PaddleOCR / OpenCV / SAM 2 / Gemini fallback); native grouping handling (groups, tables, placeholder inheritance); loss-flag taxonomy. Also adds `skills/stageflip/workflows/export-google-slides/SKILL.md` (render-diff convergence loop, T-252) and updates `skills/stageflip/workflows/export-pptx/SKILL.md` for placeholder-inheritance write-back (T-253) | M |
| **T-251 [new]** | **Schema: deck-level layouts + masters + `inheritsFrom`** — foundational schema lift for round-trip fidelity. Adds `Document.layouts: SlideLayout[]` + `Document.masters: SlideMaster[]` (or unified `slideTemplates`) and a per-element `inheritsFrom: { templateId, placeholderIdx } \| undefined`. Resolution at render time: renderer walks the inheritance chain, slide overrides winning. Must land **before** T-244 emits flat geometry, otherwise we ship a lossy import and have to migrate. Cascade through `@stageflip/schema` (Zod + types), RIR compiler, renderer-core element dispatcher, and existing `@stageflip/export-pptx` (T-253 follow-on) per CLAUDE.md §10. Includes pre-flight audits: confirm `import-pptx` canonical emit preserves `GroupElement` rather than flattening to leaves; confirm `TableElement` models merged cells (PPTX `<a:gridSpan>` / `<a:rowSpan>` / `<a:vMerge>`; Slides `MergeTableCellsRequest`) | L |
| **T-252 [new]** | **`@stageflip/export-google-slides` — render-diff convergence loop**. Peer of `@stageflip/export-pptx`. Plans `presentations.batchUpdate` requests in preference order: (1) update existing object (preserves placeholder inheritance + theme bindings), (2) duplicate similar object and modify, (3) create from scratch. After each apply, fetches the rendered slide PNG and diffs against a renderer-cdp golden; adjusts translate / size / font-size / spacing per object until tolerance is met (text bbox ±2 px position / ±3 px size; image / shape ±1 px; whole-slide perceptual diff < 1–3%) or max-iterations (default 3) is hit. Residual out-of-tolerance objects emit as image-fallback via T-245's `rasterizeFromThumbnail` with `LF-GSLIDES-EXPORT-FALLBACK`. Group export issues child creates first, then `GroupObjectsRequest`; tables use `MergeTableCellsRequest`; placeholders prefer `UpdateShapePropertiesRequest` against the inherited shape rather than creating fresh shapes. Three tier modes (fully-editable / hybrid / pixel-perfect-visual); hybrid is default. Depends on T-244 + T-245 + T-251 | L |
| **T-253-base [split]** | **`@stageflip/export-pptx` — foundational PPTX writer.** The original v1.24 row's "rider" framing assumed an existing exporter; the package was a stub. T-253-base ships the foundation: `exportPptx(doc, opts) => { bytes, lossFlags }`, ZIP layout (`[Content_Types]`, root rels, presentation, slides, theme stub, docProps, media), 4 element emitters (text / image / shape with preset / group), 6 `LF-PPTX-EXPORT-*` codes, deterministic-bytes contract, and a `roundtrip.test.ts` suite that re-imports every importer fixture and asserts structural equality. Tables / videos / embedded fonts / animations / theme write-back deferred to follow-on riders. See `docs/tasks/T-253-base.md` | M |
| **T-253-rider [split]** | **`@stageflip/export-pptx` placeholder-inheritance write-back** — the original v1.24 row scope. Adds `<p:sldLayout>` / `<p:sldMaster>` part emission mirroring `Document.layouts` / `Document.masters`; per-element `inheritsFrom` becomes `<p:nvSpPr><p:nvPr><p:ph type="..." idx="..."/></p:nvPr></p:nvSpPr>` plus override-suppression for fields equal to the placeholder default. Three new loss flags (`LF-PPTX-EXPORT-LAYOUT-NOT-FOUND`, `LF-PPTX-EXPORT-PLACEHOLDER-NOT-FOUND`, `LF-PPTX-EXPORT-PLACEHOLDER-MISMATCH`). Hard-depends on T-253-base. See `docs/tasks/T-253-rider.md` | S |
...
**γ-followups (raised by post-merge review of γ-core / γ-live tasks; ordering constraints noted per row)**

| ID | Task | Size |
|---|---|---|
| T-309a | Tighten `check-determinism` shader sub-rule — visit `MethodDeclaration` inside top-level classes under `clips/{shader,three-scene}/**`, OR drop path-based detection in favour of decorator-only opt-in. Raised by T-383 PR review (Major M-1): the path-based regime can be evaded by wrapping render-time logic in a static-only class, which T-383 did out of necessity. Must land before T-384 (`ThreeSceneClip`) so the workaround does not propagate. **DONE** — merged 2026-04-29 as PR #270. | S |
...
| T-386a | Variant-generation **size axis** — extend `VariantMatrixSpec` with a `sizes` axis (constraint-based layout adaptation across aspect ratios). Out-of-scope for T-386 by design (D-T386-8); the TypeScript `size?: never` marker on `VariantMatrixSpec` and the Zod `.strict()` reject are the placeholders. Independent of γ-live; pickable any time. | M |
| T-388a | **Static-fallback generator registry** — replace the family-hardcoded `if (clip.family !== 'voice')` branch in `mount-harness.ts:resolveStaticFallbackElements` (PR #280) with a per-family registry parallel to `interactiveClipRegistry`. Must land before T-389 (`AiChatClip`) so the next γ-live family registers a generator instead of accumulating another `if`-branch. Raised post-T-388 merge by Orchestrator inspection 2026-04-30. | S |
...
35 tasks. Authoring-time generation of video / music / 3D / TTS / SFX assets via a uniform **Provider Seam Pattern** (ADR-007, meta) — the same shape as `@stageflip/runtimes/contract`, `TranscriptionProvider`, and storage interfaces. Distinct from the frontier `AiGenerativeClip` (ADR-005 §D1) which generates at playback time; this phase produces frozen files consumed by existing `MediaElement` schema slots.
...
| ID | Task | Size |
|---|---|---|
| T-415 | ADR-007: Provider Seam Pattern (meta) — codifies adapter contract across modalities; `AdapterDescriptor`, capability descriptor, license-aware routing, sandbox model. Cross-references existing `@stageflip/runtimes/contract`, `TranscriptionProvider`, storage seams as canonical examples. **Required reading**: `docs/proposals/source-grounded-providers.md` — fold the `sourceGrounded` capability flag + `requiresResearchProvider` linkage into `AdapterDescriptor` on day one (proposal §2.1, §3.3). Shipped at `docs/decisions/ADR-007-provider-seam-pattern.md` (renumbered from plan-stated ADR-006 to resolve collision with existing `ADR-006-collab-crdt-transport.md`; v1.26 records the renumber). | S |
| T-416 | ADR-008: Asset Generation — content-addressed cache key, `MediaElement.provenance` schema, voice-consent policy, modality-specific contracts. Shipped at `docs/decisions/ADR-008-asset-generation.md`. Folds in `docs/proposals/source-grounded-providers.md` Sections 2.2 (`ResearchSessionProvider` interface body), 2.3 (`Document.research?` + `MediaProvenance.{researchSessionId, sourceIds}` schema additions), 2.4 (7 per-modality provider class bodies — `SlideDeckGenerationProvider`, `MindMapGenerationProvider`, `TableGenerationProvider`, `QuizGenerationProvider`, `FlashcardGenerationProvider`, `ReportGenerationProvider`, `InfographicGenerationProvider`), and §5 (4 `LF-RESEARCH-*` LossFlagCodes — `LF-RESEARCH-SESSION-LOST`, `LF-RESEARCH-SOURCE-UPLOAD-FAILED`, `LF-RESEARCH-PROVIDER-RATE-LIMITED`, `LF-RESEARCH-CITATIONS-MISSING`; D9 demoted reconnect from a fifth code to a UI toast). After T-415 + T-416 merge, the proposal moves to `docs/proposals/archive/source-grounded-providers.md` per its own §10 disposition; v1.27 records the archive move. | S |
...
40 tasks. Native interactive audience-engagement primitives (live polls, Q&A, quizzes, word clouds, surveys, leaderboards, plus motion-native differentiators) for stageflip-slide and stageflip-display, with Slido / Mentimeter / Poll Everywhere bridges via the same ADR-007 provider seam from P14. Frozen-fallback exports across MP4 / PPTX / display via the `staticFallback` contract (ADR-005 §D2).
...
| ID | Task | Size |
|---|---|---|
| T-450 | ADR-009: Audience Backend — cites ADR-007; native + vendor adapter contract; real-time persistence model; rate limits; SLA target (1000 concurrent voters v1). (Renumbered from plan-stated ADR-008 per v1.26 — the original ADR-006/007 numbers shifted to ADR-007/008 to resolve the existing-ADR-006 collision.) Shipped at `docs/decisions/ADR-009-audience-backend.md` (~668 lines, 13 decisions D1–D13); v1.28 records the merge. First Phase 15 task + first of two Phase 15 α hard-gate ADRs (T-451 / ADR-010 follows). | S |
| T-451 | ADR-010: Live Audience Clip Family — the 9 v1 clips; `staticFallback` snapshot semantics; `AudienceProvenance` schema. (Renumbered from plan-stated ADR-009 per v1.26.) Shipped at `docs/decisions/ADR-010-live-audience-clip-family.md` (~815 lines, 13 decisions D1–D13 incl. §D3a snapshot-size hygiene); v1.29 records the merge. **Phase 15 α hard gate cleared** (together with T-450 / ADR-009 merged via PR #512 / v1.28); T-452+ can dispatch. | S |
...
| ID | Task | Size |
|---|---|---|
| T-490 | ADR-012: Bundle Format & License Runtime — manifest spec, signature scheme, license-claim format, version compatibility, license-runtime enforcement. (Renumbered from plan-stated ADR-011 per v1.26.) | M |
| T-491 | ADR-013: First-party Pack Catalogue & Pricing Tiers — what we sell, pricing logic, free / paid / enterprise tier structure. (Renumbered from plan-stated ADR-012 per v1.26.) | M |
| T-492 | ADR-014: Marketplace — decision on dedicated registry vs npm + scoped + auth. (Renumbered from plan-stated ADR-013 per v1.26.) | S |
...
- **v1.29** (2026-05-11): **ADR-010 (Live Audience Clip Family) shipped — Phase 15 α hard gate CLEARED.** T-451 lands `docs/decisions/ADR-010-live-audience-clip-family.md` (~815 lines, 13 decisions D1–D13 incl. §D3a snapshot-size hygiene) — the second of two Phase 15 α hard-gate ADRs. **Together with ADR-009 (T-450, merged via PR #512 / v1.28), this clears the Phase 15 α hard gate; T-452+ can dispatch** — `@stageflip/audience-contract` (T-452), the audience backend service (T-453), `packages/runtimes/audience/` (T-454), `check-audience-permissions` (T-455), audience-join UX (T-456), `tools/audience-engagement/SKILL.md` (T-457), rate-limit / spam protection (T-458), result-export (T-459), `AudienceProvenance` type (T-460), the nine v1 clip family implementations (T-461 → T-471), static-fallback consolidation (T-472), quiz fairness (T-473), audience-data persistence (T-474), latency tests (T-475), Cluster I parity fixtures (T-476), SLA load test (T-477), the five vendor adapters (T-479 → T-483), WebEmbed allowlist update (T-484), vendor adapter regression suite (T-485), Cluster I preset cluster (T-486), Cluster I SKILL + compose_* tools (T-487), GA readiness + security review (T-488), Phase 15 closeout handover (T-489) — all unblocked. ADR-010 defines: §D1 nine v1 audience clip families spanning eleven `AudienceClipKind` discriminants (six standard: LivePoll multi-choice/open-text/rating, LiveQA, LiveQuiz, Leaderboard, WordCloud, Survey; three motion-native differentiators: Heatmap / ReactionStream / AudienceAiPrompt — Slido / Mentimeter / Poll Everywhere / Vevox / Wooclap structurally cannot reach the three motion-native ones); §D2 per-clip-kind `VotePayload.value` shapes (typed discriminated union — `LivePollMultipleChoiceVote` / `LivePollOpenTextVote` / `LivePollRatingVote` / `LiveQAVote` / `LiveQuizVote` / `LeaderboardVote = never` / `WordCloudVote` / `SurveyVote` / `HeatmapVote` / `ReactionStreamVote` / `AudienceAiPromptVote`); §D3 per-clip-kind `AggregationSnapshot.aggregation` shapes (typed discriminated union — eleven aggregation variants, each carrying the client-ready aggregation); §D3a cross-clip snapshot-size hygiene table (per-kind upper bounds 1 KB..200 KB; downsampling on cap rather than document rejection); §D4 `staticFallback` snapshot semantics (when snapshot is taken — at session close as `FinalSnapshot` per ADR-009 §D2, plus periodic snapshots at adapter cadence; what bytes/JSON the persisted snapshot carries; how renderer materializes a frame per clip family — pure layout passes for LivePoll/LiveQA/LiveQuiz/Leaderboard/Survey/WordCloud; raster blend for Heatmap; ShaderClip pass at peak for ReactionStream; P14-cache fetch for AudienceAiPrompt; three snapshot-selection policies: `final` / `peak` / `at-frame`); §D5 full `AudienceProvenance` schema — extends ADR-009 §D9 preview slots (`provider`, `sessionId`, `snapshotFrame`, `voterCountAtCapture`, `capturedAt`) with `snapshotPolicy` + `clipKind` + inlined `aggregation: AggregationValue` payload (mirrors `MediaProvenance` posture from ADR-008 §D2 — inlined-bytes-for-export-durability so exported slides remain renderable after Firestore session TTL); §D6 `permissions: ['audience-network']` requirement on every audience clip per ADR-005 §D2 manifest convention (T-455 CI gate enforces); §D7 motion-native dependency footprint — ReactionStreamClip depends on **T-383** (`ShaderClip`, P13 γ); AudienceAiPromptClip depends on **T-430** (Seedance video adapter) + **T-432** (ACE-Step music adapter), both P14 β; §D8 static-fallback vs. live-mount routing posture (three-state model: live / static-fallback / empty live-mount — the editor's authoring-defaults state is distinct from the two-path renderer contract per ADR-005 §D2); §D9 tenant frontier-enablement gating — `TenantSettings.features.audience` sub-namespace extends the T-411a tenant-settings facet with two-flag enablement (`enabled` + `motionNativeEnabled`) for phased rollout (motion-native readiness independent of standard-clip readiness; matches multi-flag-not-tri-state rollout reality); §D10 Cluster I preset cluster (~6 audience presets: `slido-classic-poll` / `mentimeter-bar-vote` / `kahoot-competitive` / `bbc-question-time` / `conference-qa-upvote` / `classroom-quiz` — T-486); §D11 determinism posture inherited from ADR-009 §D10 (audience runtime sits inside the determinism perimeter per CLAUDE.md §3; `staticFallback` deterministic-by-construction given identical snapshot bytes; live path observably non-deterministic; per-clip-kind determinism notes including ShaderClip and CanvasElement inheritance per ADR-005 §D2); §D12 loss-flag inventory inherited from ADR-009 §D11 (no new flags; the eight `LF-AUDIENCE-*` codes cover all clip-family failure modes); §D13 plugin manifest extensions inherited (third-party-plugin audience-clip-family contribution kind explicitly out-of-scope for v1; punted to Phase 16 marketplace). Eight rejected alternatives (A: unified LivePoll variants under a single clip family; B: polymorphic submitVote methods; C: pre-rasterized HeatmapClip snapshots; D: pointer-only `AudienceProvenance.aggregation`; E: two-state model without empty live-mount; F: unified tri-state `features.audience.tier`; G: unify `AudienceProvenance` with `MediaProvenance`; H: defer per-clip-kind shapes to T-452; I: motion-native differentiator count). **§13 (structural extension) statement**: NOT a structural extension — pure docs ADR. The schema additions ADR-010 specifies (`VotePayloadValue` + `AggregationValue` discriminated unions, `AudienceProvenance` schema, per-clip-family manifests + clip-family implementations) land in T-452 (`@stageflip/audience-contract`) + T-460 (`AudienceProvenance` type) + T-461..T-471 (the nine clip family implementations) which DO bear the §13 obligation; T-476 Cluster I parity fixtures + the PO ratification sign-off carry the end-to-end render verification per CLAUDE.md §13 means-of-verification (option 2 — reference preset/fixture sign-off via standard parity-fixture flow). Sub-task spec at `docs/tasks/T-451.md`. No code, fixture, package, or skill changes in this PR.
- **v1.28** (2026-05-11): **ADR-009 (Audience Backend) shipped — Phase 15 α opened.** T-450 lands `docs/decisions/ADR-009-audience-backend.md` (~668 lines, 13 decisions D1–D13) — the first of two Phase 15 α hard-gate ADRs (T-451 / ADR-010 follows; together they block T-452+). ADR-009 is the **third downstream consumer** of ADR-007's Provider Seam Pattern (after ADR-008 asset-gen): defines the Live Audience backend architecture for native + vendor (Slido / Mentimeter / Poll Everywhere / Vevox / Wooclap) audience-engagement primitives. Decisions: §D1 backend service architecture (WebSocket multiplexer at `/v1/audience/ws/:sessionId` + Firestore `audience-sessions` collection — extends `apps/api`, not a separate deployment); §D2 `AudienceBackendProvider` interface extending `AdapterDescriptor` with four methods (`openSession` / `submitVote` / `subscribe` / `closeSession`) + `AudienceCapabilityDescriptor` (persistenceTier / maxConcurrentVoters / supportedClipKinds / supportsMotionNative / voterIdentity / supportsStaticFallback / maxIngestRateHz / snapshotCadenceHz); §D3 three-layer rate-limit model (per-tenant + per-session voter cap + per-voter anti-spam, with drop-not-queue backpressure); §D4 SLA target (1000 concurrent voters v1; p50 < 200 ms / p95 < 500 ms voter-tap → screen, matching T-475's explicit acceptance criterion); §D5 persistence model (Firestore `tenants/{tenantId}/projects/{projectId}/audience-sessions/{sessionId}` with `events/` + `snapshots/` sub-collections, TTL policy, voter-token hashing at rest, 30 Hz default snapshot cadence with 5 Hz per-kind override for ReactionStream); §D6 WebSocket transport (lifecycle, exponential backoff reconnect — 6 attempts × max 30 s, heartbeat); §D7 dual-trust-domain auth (presenter admin token + anonymous voter session token); §D8 vendor adapter bridge contract + illustrative vendor parity matrix (motion-native = native-only for v1 — three differentiators exploit motion + 3D + frontier stack Slido/Mentimeter/etc. structurally cannot reach); §D9 `AudienceProvenance` schema **preview** (full schema lands in ADR-010 + T-460); §D10 determinism posture (live path observably non-deterministic; staticFallback contractually deterministic given identical snapshot); §D11 eight `LF-AUDIENCE-*` loss-flag codes; §D12 plugin manifest per-modality extensions (audience-backend-provider); §D13 permissions manifest cross-reference (`audience-network` per ADR-005 + T-455). **§13 (structural extension) statement**: NOT a structural extension — pure docs ADR. The schema additions ADR-009 / ADR-010 specify land in T-452 (`@stageflip/audience-contract`) + T-460 (`AudienceProvenance` type) which DO bear the §13 obligation. Sub-task spec at `docs/tasks/T-450.md`. No code, fixture, package, or skill changes in this PR.
- **v1.27** (2026-05-11): **ADR-008 (Asset Generation contract) shipped — Phase 14 α hard gate cleared.** T-416 lands `docs/decisions/ADR-008-asset-generation.md` (~786 lines, 13 decisions D1–D13) — the second of two Phase 14 α hard-gate ADRs. Together with ADR-007 (T-415, merged 2026-05-11 via PR #478), this clears the hard gate; downstream Phase 14 α tasks T-417 (concept SKILL), T-418 (`@stageflip/adapters-core`), T-419 (`@stageflip/asset-gen-contract`), T-420 (`@stageflip/asset-cache`), T-421 (`MediaElement.provenance` schema), T-422 (`check-asset-licenses`), T-423 (`tools/asset-generation/SKILL.md`), T-424 (adapter catalog), T-425 (capability-routing engine) can dispatch. ADR-008 defines: §D1 content-addressed cache key (SHA-256 over canonicalized `{modality, model, voice, prompt, params, seed}`); §D2 `MediaProvenance` strict-discriminated shape (kind / provider / model / prompt / cacheKey / seed / voiceProvider / voiceId / clonedFromConsent / researchSessionId / sourceIds); §D3 `Document.research?: ResearchSessionRef` slot; §D4 voice-consent policy (per-call check, tenant-admin-declared, dedicated `TenantVoiceConsentStore` storage facet matching the `TenantSettingsStore` carve-out pattern from T-411a); §D5 + §D6 five β modality contracts (TTS / video-gen / music-gen / SFX / 3D) extending `AdapterDescriptor`; §D7 `ResearchSessionProvider` asset-gen wiring (ping → reconnect → call → cite-or-fall-through routing); §D9 seven source-grounded provider class bodies preserved from ADR-007 §D6 scope; §D11 four `LF-RESEARCH-*` codes + two `LF-VOICE-CONSENT-*` codes; §D12 plugin manifest per-modality extensions; §D13 per-modality license-posture preview (T-422 implements). **Folds in `docs/proposals/source-grounded-providers.md` Sections 2.2 / 2.3 / 2.4 / §5** — completing the absorption begun by ADR-007 (which absorbed §2.1 + the enumeration + design decisions D1–D9). The proposal is **archived** in this PR per its own §10 disposition: moved from `docs/proposals/source-grounded-providers.md` → `docs/proposals/archive/source-grounded-providers.md`. **§13 (structural extension) statement**: NOT a structural extension — pure docs ADR. The schema additions ADR-008 specifies (`Document.research?`, `MediaProvenance` slot) land in T-419 / T-421 which DO bear the §13 obligation; their PRs ship roundtrip tests proving the additions are non-breaking. Sub-task spec at `docs/tasks/T-416.md`. No code, fixture, package, or skill changes in this PR.
- **v1.26** (2026-05-11): **Phase 14+ ADRs renumbered to resolve collision with existing ADR-006-collab-crdt-transport.** Plan v1.22 / v1.24 row text on T-415 / T-416 referred to "ADR-006 (Provider Seam Pattern)" and "ADR-007 (Asset Generation)" — both not-yet-written. `docs/decisions/ADR-006-collab-crdt-transport.md` had already shipped (T-260 / Phase 12, ratified 2026-04-27 via `d32aea5`). T-415's Implementer dispatch surfaced the collision. Renumber decision: shift every Phase 14+ ADR forward by one. **T-415** → ADR-007 (Provider Seam Pattern, meta) — shipped this PR at `docs/decisions/ADR-007-provider-seam-pattern.md`. **T-416** → ADR-008 (Asset Generation contract). **T-450** → ADR-009 (Audience Backend). **T-451** → ADR-010 (Live Audience Clip Family). **T-490 / T-491 / T-492** → ADR-012 / ADR-013 / ADR-014 (Bundle Format & License Runtime / First-party Pack Catalogue / Marketplace) — preserves the existing-ADR-006 gap-free numbering downstream. The existing `ADR-006-collab-crdt-transport.md` is **untouched** (preserved verbatim per task constraint). Plan-row text on T-415, T-416, T-450, T-451, T-490, T-491, T-492 amended; v1.22 + v1.24 changelog entries cross-referenced. **Known forward-cite stragglers (out-of-scope follow-up)**: `skills/stageflip/runtimes/{ai-chat,live-data,voice,ai-generative}/SKILL.md`, `skills/stageflip/concepts/{variants,runtimes}/SKILL.md`, and `docs/tasks/{T-389,T-391,T-395,T-410}.md` still reference "Phase 14 ADR-006" intending the asset-gen pattern; these are forward-references to ADRs that did not exist at the time and should be updated to ADR-007 / ADR-008 in a follow-up sweep (zero impact on plan correctness; the renumber landing in this PR is the source of truth). T-415 also ships sub-task spec at `docs/tasks/T-415.md` and folds-into pointer in `docs/proposals/source-grounded-providers.md`. No code, fixture, package, or skill changes in this PR.
- **v1.23** (2026-04-26): **Google Slides import/export — pixel-perfect import + native grouping + new export task.** T-244 [rev], T-245 [rev], T-246 [rev], T-250 [rev]; new rows T-251 (schema: deck-level layouts/masters + `inheritsFrom`), T-252 (`@stageflip/export-google-slides` with render-diff convergence loop), T-253 (`@stageflip/export-pptx` placeholder-inheritance rider). Three pillars: (1) **pixel-perfect import** — Slides API geometry alone isn't pixel-accurate (server-side autofit, padding, font substitution, theme inheritance), so T-244 pairs `presentations.get` with the rendered slide PNG and recovers visible boxes via PaddleOCR + OpenCV (+ optional SAM 2); Gemini reserved as per-element fallback (T-246 inverted: classical CV first, ~90% deterministic resolution, Gemini for the residual). (2) **Native grouping for round-trip** — Slides has three "kinds" (explicit `elementGroup`, tables, placeholder inheritance) with structurally identical PPTX analogues (`<p:grpSp>`, `<a:tbl>`, `<p:sldLayout>` / `<p:sldMaster>`); two are already in our schema (`GroupElement` at `packages/schema/src/elements/index.ts:25`, `TableElement`), the third (placeholder inheritance) is the foundational schema lift in T-251. (3) **Google Slides export** — currently absent from the plan; T-252 mirrors `@stageflip/export-pptx` with a render-diff convergence loop because Slides server-side rendering will silently drift from any one-shot batchUpdate write. T-245 promoted from "crop unsupported shapes" to a shared rasterization primitive used by both import and export image-fallback paths. **T-251 must land before T-244** emits flat geometry — otherwise we ship a lossy import and have to migrate. Originally drafted as v1.21; bumped to v1.23 during rebase since v1.21 (T-242d / T-247-loss-flags) and v1.22 (P14 / P15 / P16 scoping) landed first.
- **v1.25** (2026-04-26): **T-253 split: T-253-base + T-253-rider.** The v1.23 row at `docs/implementation-plan.md:542` framed T-253 as a "rider" updating the existing exporter, but `@stageflip/export-pptx` was a stub (only `export {}` since T-011 scaffolding). T-253 now splits: **T-253-base (M)** ships the foundational PPTX writer (`exportPptx`, ZIP layout, 4 element emitters, 6 `LF-PPTX-EXPORT-*` codes, deterministic-bytes contract, round-trip suite); **T-253-rider (S)** ships the original v1.23 scope (`<p:sldLayout>` / `<p:sldMaster>` parts + `<p:ph>` per-element references + override-suppression + 3 new loss flags). Specs at `docs/tasks/T-253-base.md` + `docs/tasks/T-253-rider.md`. Tables / videos / embedded fonts / theme write-back are explicitly deferred to follow-on riders (independent of T-253-rider). No code, fixture, or skill changes in this revision.
- **v1.24** (2026-04-26): **Source-grounded providers proposal referenced from T-415 / T-416.** Added "Required reading" callouts on the two Phase 14 α ADR rows pointing at `docs/proposals/source-grounded-providers.md` (PR #197). Proposal extends the two Phase 14 α ADRs (originally numbered ADR-006/007 in this changelog entry; renumbered to ADR-007/008 in v1.26) with: (a) `sourceGrounded` capability flag + `requiresResearchProvider` linkage on `AdapterDescriptor`, (b) new `ResearchSessionProvider` meta-interface (session-scoped, multi-modal — NotebookLM is canonical example, future Perplexity Pro / Claude Projects fit the same shape), (c) 7 new per-modality provider classes (`SlideDeckGenerationProvider`, `MindMapGenerationProvider`, `TableGenerationProvider`, `QuizGenerationProvider`, `FlashcardGenerationProvider`, `ReportGenerationProvider`, `InfographicGenerationProvider`), (d) `Document.research?: ResearchSessionRef` + `MediaProvenance.{researchSessionId, sourceIds}` schema additions, (e) 5 new `LF-RESEARCH-*` LossFlagCodes for session lifecycle events (one demoted to a UI toast per D9 → 4 codes net). Five design decisions ratified up-front in the proposal (D1: source replace-in-place with content-hash provenance; D2: project-lifetime sessions; D3: optional grounding gracefully falls through to creative providers; D4: one research provider per project for v1; D5: per-modality adapter declares `requiresResearchProvider` for manifest-time linkage). Proposal explicitly NOT a separate ADR — must fold into the two Phase 14 α ADRs directly when T-415/T-416 dispatch (otherwise those ADRs would need rewriting). After absorption, proposal moves to `docs/proposals/archive/`. No code, fixture, or skill changes in this revision.
- **v1.22** (2026-04-26): **Phases 14, 15, 16 scoped.** 140 new tasks (T-415 → T-554) added across three downstream phases. **Phase 14 — Asset Generation** (35 tasks, T-415 → T-449): authoring-time generation of video / music / 3D / TTS / SFX assets via a uniform Provider Seam Pattern (ADR-006 meta + ADR-007 asset-gen contract — both renumbered to ADR-007 / ADR-008 per v1.26 to resolve the existing-ADR-006 collision). 9 reference adapters in v1 (Kokoro / Fish Speech / Tripo / Meshy / Seedance 2.0 / Runway / ACE-Step / YuE / Stable Audio Open). New canonical tool bundle #18 (`asset-generation`); cross-cutting integrations include TTS↔captions bypass-Whisper path (eliminates Whisper cost on synthesized audio) and 3D↔ThreeSceneClip GLB consumer. **Phase 15 — Live Audience** (40 tasks, T-450 → T-489): native interactive audience-engagement primitives — 6 standard clips (LivePoll / LiveQA / LiveQuiz / WordCloud / Survey / Leaderboard) + 3 marquee motion-native differentiators (Heatmap / ReactionStream / AudienceAiPrompt) Slido cannot structurally reach; vendor adapter bridges (Slido / Mentimeter / Poll Everywhere / Vevox / Wooclap) via the same ADR-007 seam (renumbered per v1.26); Cluster I preset cluster (~6 presets). New canonical tool bundle #19 (`audience-engagement`). Audience backend service extends `apps/api` with WebSockets + Firestore audience-results collection. **Phase 16 — Bundles & Marketplace** (65 tasks, T-490 → T-554): OSS-path optionality — turns the skill-tree + adapter architecture into a marketplace of paid content (preset packs / cluster extensions / adapter packs / asset packs / theme packs / composition templates). Six first-party launch packs (News Pro / Sports Networks / Creator Style / Earnings & Investor / Wedding & Events / Frontier Effects). License runtime gates paywall-locked clips at instantiation; OSS core + paid content coexist. **Provider Seam Pattern (ADR-007 — renumbered per v1.26 from plan-stated ADR-006)** is the meta-ADR codifying the adapter pattern across all of P14, P15, P16, retroactively cross-referencing the existing `@stageflip/runtimes/contract`, `TranscriptionProvider`, storage seams as canonical examples. Total plan scope: 530+ tasks across 16 phases. **Plan-only changes; zero clip / runtime / package code in this PR.** P13–P16 are structurally parallel; P11 retains capacity priority. ADR ratification + per-phase implementation are separate PRs following the standard three-agent workflow.
...
**End of plan v1.29.** Start at T-001.
