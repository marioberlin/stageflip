...
| T-416 | ADR-008: Asset Generation — content-addressed cache key, `MediaElement.provenance` schema, voice-consent policy, modality-specific contracts. **Required reading**: `docs/proposals/source-grounded-providers.md` — fold the `ResearchSessionProvider` meta-interface (§2.2), 7 new per-modality provider classes (§2.4), `Document.research` + `MediaProvenance.{researchSessionId, sourceIds}` schema additions (§2.3), and 4 `LF-RESEARCH-*` LossFlagCodes (§5; D9 demoted reconnect from loss flag to UI toast) into ADR-008 directly — NOT as a separate ADR. | S |
...
**End of plan v1.26.** Start at T-001.
