...
| **T-253 [new]** | **`@stageflip/export-pptx` placeholder-inheritance rider** — update existing exporter to write `<p:sldLayout>` / `<p:sldMaster>` parts mirroring the canonical `Document.layouts` / `Document.masters` rather than inlining theme values onto every slide. Per-element `inheritsFrom` becomes `<p:nvSpPr><p:nvPr><p:ph type="..." idx="..."/></p:nvPr></p:nvSpPr>`. Independent of T-252 timing; opportunistic follow-on once T-251 lands | M |
