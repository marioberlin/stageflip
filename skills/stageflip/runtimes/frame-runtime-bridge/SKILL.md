...
cdp-host-bundle passes to `createFrameRuntimeBridge`. All 54 bridge
...
`follow-prompt` primitive) — see the tranche ledger below for the
breakdown.
...
| `src/clips/index.ts` | T-131b/d/e/f, T-183, T-202, T-316, T-317, T-318, T-321, T-322, T-324a, T-332a, T-355a, T-356a, T-357a, T-358a, T-406 | Barrel + `ALL_BRIDGE_CLIPS` constant (54 clips: 32 reference-clip ports + 10 StageFlip.Video / StageFlip.Display profile clips + the unified `chart` family + the `outcome-row` primitive + the `news-ticker-bar` primitive + the `standings-table` primitive + the `caption` primitive + the `magic-wall-panel` primitive + the `lyrics` primitive + the `title-sequence` primitive + the `breaking-banner` primitive + the `score-bug` primitive + the `subscribe-button` primitive + the `follow-prompt` primitive) |
