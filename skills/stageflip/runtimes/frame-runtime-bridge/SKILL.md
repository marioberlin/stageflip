...
| kind | file | notes |
|---|---|---|
| `link-sticker` | `src/clips/link-sticker.tsx` | Single-register Cluster G primitive serving the Instagram-style link-sticker canon (`instagram-link-sticker.md`, `clipKind: socialMedia`) — a rounded-pill (~200 × 44 px native) free-form-positioned link sticker on a Story frame with a closed-form linear shimmer / highlight sweep across the label glyphs (3 s default cycle). Single Zod `object().strict()` schema (NO `discriminatedUnion` — variant-specific fields are minimal) with sealed `variant: z.enum(['white-on-dark', 'dark-on-white', 'frosted-glass', 'brand-color'])` and sealed `phase: z.enum(['idle', 'shimmering'])`. Required props: `label` (1–80 chars), `variant`, `position: { x, y }` (free-form, no anchor canon — top-left of pill in canvas pixels). Optional: `phase` (default `'shimmering'` — the canonical mid-shimmer reference frame is frame 60), `width` (80–600, default 200), `height` (28–96, default 44), `fontSize` (10–24, default 14), `brandColor` (only meaningful when `variant === 'brand-color'`), `shimmer.cycleFrames` (default `ceil(fps * 3)` = 90 at fps 30), `shimmer.bandWidth` (default 40), `shimmer.highlightColor` (default per-variant from `VARIANT_TOKENS`), per-slot color overrides (`background` / `textColor` / `shadowColor`). **Shimmer math** (per D-T371a-4): closed-form linear sweep — `shimmerX(f) = round(((f % cycleFrames) / cycleFrames) * (pillWidth + bandWidth) - bandWidth)`. At frame 0 → `-bandWidth` (band off-left); at frame `cycleFrames/2` → centered-ish; at frame `cycleFrames` → wraps back. Pure trigonometry; integer-pixel positions (`Math.round` at final step). **Per-variant token table** (D-T371a-5): `'white-on-dark'` → black background + white text + black shadow; `'dark-on-white'` → white background + black text + grey shadow; `'frosted-glass'` → opaque `#CCCCCC` fallback (D-T371a-blur deferral — `backdrop-filter: blur()` is not deterministic across CDP versions) + black text + grey shadow; `'brand-color'` → `brandColor` prop or default `#E1306C` (Instagram pink) + white text + black shadow. Resolution: consumer prop > brand-color override (when `variant === 'brand-color'` AND `brandColor` set AND `background` unset) > variant default. **Geometry**: rounded pill with `border-radius = height / 2` (full-pill); `position: absolute` at `(position.x, position.y)`; `box-shadow: 0 2px 4px <shadowColor>`. **Shimmer rendering** (when `phase === 'shimmering'`): inline `<div>` overlay inside the pill at `left: <shimmerX>px; width: <bandWidth>px; height: 100%; background: linear-gradient(90deg, transparent 0%, <highlightColor> 50%, transparent 100%); pointer-events: none`. When `phase === 'idle'`, shimmer overlay skipped entirely. Frame-deterministic — no `Date.now` / `Math.random` / `crypto.randomUUID` / `setTimeout` / `setInterval` / `requestAnimationFrame` / `fetch` / `addEventListener`. NO SVG instance-IDs in v1 (plain `<div>` rendering). Inter Medium (OFL, T-307) registered as the hard fallback font via `fontRequirements: () => [{ family: 'Inter', weight: 500 }]`; the Instagram proprietary system font is `platform-byo` (consumer-wired via `runtime.fonts`). Theme-slot fallback (`background` → `palette.background`, `textColor` → `palette.foreground`, `shadowColor` → `palette.foreground`). v1 carve-outs: tap-depress to 95 % scale + link-preview card from bottom (T-371a-followup; reference frame 90 deferred), `backdrop-filter: blur(...)` for `'frosted-glass'` (T-371a-blur — CDP determinism hazard; flat translucent fallback in v1), additional sticker kinds — mention / poll / GIF / question / slider / music (T-371a-extend), real Instagram-domain icon SVG (T-371a-glyph; v1 ships pure text or placeholder generic-link glyph). Sibling Cluster G primitives: T-317 `subscribe-button` (horizontal pill register; landed), T-318 `follow-prompt` (circular avatar register; landed), T-319 `qr-code-bounce` (DVD-screensaver bouncing QR; landed). Unblocks T-371 (`instagram-link-sticker`, Cluster G's last unsigned preset; primary v1 consumer); PSNR ≥ 42 dB / SSIM ≥ 0.98 acceptance bar (cluster-norm). With T-371a merged, all four Cluster G blocking primitives are shipped (T-317 + T-318 + T-319 + T-371a) — T-371a is the fourth and final Cluster G blocking primitive. Cluster-specific palettes / labels / position offsets live in `parity-cli` resolver shims, not in this primitive. |
...
cdp-host-bundle passes to `createFrameRuntimeBridge`. All 56 bridge
...
the T-371a `link-sticker` primitive) — see the tranche ledger below
for the breakdown.
...
| `src/clips/index.ts` | T-131b/d/e/f, T-183, T-202, T-316, T-317, T-318, T-319, T-321, T-322, T-324a, T-332a, T-355a, T-356a, T-357a, T-358a, T-371a, T-406 | Barrel + `ALL_BRIDGE_CLIPS` constant (56 clips: 32 reference-clip ports + 10 StageFlip.Video / StageFlip.Display profile clips + the unified `chart` family + the `outcome-row` primitive + the `news-ticker-bar` primitive + the `standings-table` primitive + the `caption` primitive + the `magic-wall-panel` primitive + the `lyrics` primitive + the `title-sequence` primitive + the `breaking-banner` primitive + the `score-bug` primitive + the `subscribe-button` primitive + the `follow-prompt` primitive + the `qr-code-bounce` primitive + the `link-sticker` primitive) |
...
| Link-sticker primitive | T-371a | link-sticker | Single-register Cluster G primitive serving the Instagram-style link-sticker canon (`instagram-link-sticker.md`, `clipKind: socialMedia`) — rounded-pill (~200 × 44 px native) free-form-positioned link sticker on a Story frame with closed-form linear shimmer / highlight sweep across the label glyphs (3 s default cycle). Single Zod `object().strict()` schema (NO `discriminatedUnion`); sealed `variant` enum (4 values: `'white-on-dark' \| 'dark-on-white' \| 'frosted-glass' \| 'brand-color'`); sealed `phase` enum (2 values: `'idle' \| 'shimmering'`). Required: `label` (1–80 chars), `variant`, `position: { x, y }` (free-form). Optional: `phase` (default `'shimmering'`), `width` (80–600, default 200), `height` (28–96, default 44), `fontSize` (10–24, default 14), `brandColor`, `shimmer: { cycleFrames, bandWidth, highlightColor }` (defaults `ceil(fps*3)` / 40 / per-variant), per-slot color overrides (`background` / `textColor` / `shadowColor`). Closed-form shimmer math `shimmerX(f) = round(((f % cycleFrames) / cycleFrames) * (pillWidth + bandWidth) - bandWidth)`. Per-variant token table (D-T371a-5): `'white-on-dark'` black/white/black; `'dark-on-white'` white/black/grey; `'frosted-glass'` opaque `#CCCCCC` fallback (D-T371a-blur deferral — `backdrop-filter: blur` is not deterministic across CDP versions); `'brand-color'` `brandColor` or default `#E1306C` (Instagram pink) / white / black. Resolution: consumer prop > brand-color override (when `variant === 'brand-color'`) > variant default. Pill geometry: `border-radius = height / 2`; `position: absolute`; `box-shadow: 0 2px 4px <shadowColor>`. Frame-deterministic — no `Date.now` / `Math.random` / `crypto.randomUUID` / `setTimeout` / `setInterval` / `requestAnimationFrame` / `fetch` / `addEventListener`. NO SVG instance-IDs in v1. Inter Medium (OFL, T-307) registered as fallback font; Instagram proprietary font is `platform-byo`. Theme-slot fallback (`background` → `palette.background`, `textColor` → `palette.foreground`, `shadowColor` → `palette.foreground`). v1 carve-outs: tap-depress + link-preview card (T-371a-followup; reference frame 90 deferred), backdrop-filter blur for `'frosted-glass'` (T-371a-blur), additional sticker kinds — mention / poll / GIF / question / slider / music (T-371a-extend), real Instagram-domain icon SVG (T-371a-glyph). Sibling Cluster G primitives: T-317 `subscribe-button`, T-318 `follow-prompt`, T-319 `qr-code-bounce`. Unblocks T-371 (`instagram-link-sticker`, Cluster G's last unsigned preset; primary v1 consumer); PSNR ≥ 42 dB / SSIM ≥ 0.98 cluster-norm. With T-371a merged, all four Cluster G blocking primitives are shipped (T-317 + T-318 + T-319 + T-371a) — fourth and final Cluster G blocking primitive. Cluster-specific palettes / labels / position offsets live in `parity-cli` resolver shims, not in this primitive. |
