...
cdp-host-bundle passes to `createFrameRuntimeBridge`. All 48 bridge
...
the T-355a `magic-wall-panel` primitive) — see the tranche ledger
...
| `src/clips/index.ts` | T-131b/d/e/f, T-183, T-202, T-316, T-355a, T-356a, T-357a, T-358a, T-406 | Barrel + `ALL_BRIDGE_CLIPS` constant (48 clips: 32 reference-clip ports + 10 StageFlip.Video / StageFlip.Display profile clips + the unified `chart` family + the `outcome-row` primitive + the `news-ticker-bar` primitive + the `standings-table` primitive + the `caption` primitive + the `magic-wall-panel` primitive) |
