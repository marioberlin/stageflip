...
9 adapters registered (vendor-neutral; per-tenant license-posture filtering happens in the routing engine, T-425).
...
|---|---|---|---|---|---|---|
| `ace-step` | music-gen | max 300s, 44100Hz | mit | free | fast < 10s | no |
| `yue` | music-gen | max 300s, 44100Hz | apache-2.0 | free | fast < 10s | no |
| `stable-audio-open` | sfx | max 30s, 44100Hz | apache-2.0 | free | fast < 10s | no |
...
| `runway` | video-gen | 16:9/9:16/1:1/4:3/3:4, max 10s | proprietary-byo | enterprise | batch < 5min | no |
